...
import React, { useEffect, useState, useContext } from 'react';
import { AuthContext } from '../../../context/AuthProvider';
import { Phone, Mail, DollarSign, User, Calendar, MapPin, MessageSquare, ShieldCheck } from 'lucide-react';
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto">
            ...
            <div className="mb-10 text-center md:text-left">
                <h2 className="text-4xl font-black text-gray-800 tracking-tight">
                    Active <span className="text-emerald-600">Learning Sessions</span>
                </h2>
                <p className="text-gray-500 font-medium mt-2">Manage your student's contact details and session info</p>
            </div>

            {tuitions.length === 0 ? (
                <div className="bg-white p-20 text-center rounded-[3rem] border-2 border-dashed border-gray-100 shadow-sm flex flex-col items-center">
                    <div className="bg-emerald-50 p-6 rounded-full mb-6 text-emerald-300">
                        <User size={48} />
                    </div>
                    <h3 className="text-xl font-bold text-gray-600">No active students yet!</h3>
                    <p className="text-gray-400 mt-2">When a student pays and confirms you, their details will appear here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {tuitions.map((t) => (
                        <div key={t._id} className="bg-white rounded-[2.5rem] shadow-lg border border-gray-50 overflow-hidden hover:shadow-2xl transition-all duration-300">
                            
                            {/* --- Card Header: Tuition Title --- */}
                            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 text-white relative">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest bg-black/20 px-2 py-1 rounded-md">Ongoing</span>
                                <h3 className="text-xl font-black mt-2 leading-tight pr-8">{t.tuitionTitle}</h3>
                            </div>
                            ...
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    {/* Name & Role */}
                                    <div className="flex items-center gap-4 p-3 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                                        <div className="p-3 bg-white rounded-xl shadow-sm text-emerald-600">
                                            <User size={20} />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter">Student Name</p>
                                            <h4 className="text-base font-bold text-gray-800">{t.studentName || "N/A"}</h4>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        ...
                                        <div className="flex items-center gap-3 text-gray-600 hover:text-emerald-600 transition-colors">
                                            ...
                                            <span className="text-sm font-medium truncate">{t.studentEmail}</span>
                                        </div>
                                        
                                        {/* Phone Number */}
                                        <div className="flex items-center gap-3 text-gray-600">
                                            <div className="p-2 bg-gray-50 rounded-lg text-emerald-600"><Phone size={16} /></div>
                                            <span className="text-sm font-bold tracking-wider">{t.studentPhone || "No Number Found"}</span>
                                        </div>

                                        {/* Salary */}
                                        <div className="flex items-center gap-3 text-gray-600">
                                            <div className="p-2 bg-emerald-50 rounded-lg text-emerald-600"><DollarSign size={16} /></div>
                                            <span className="text-sm font-black">Monthly: ৳{t.salary || t.expectedSalary}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-8 flex gap-3">
                                    <a 
                                        ...
                                        className="flex-[3] bg-emerald-600 hover:bg-emerald-700 text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 text-sm uppercase tracking-widest"
                                    >
                                        <Phone size={18} /> Call Now
                                    </a>
                                    <button 
                                        onClick={() => window.location.href = `mailto:${t.studentEmail}`}
                                        className="flex-1 bg-gray-100 hover:bg-emerald-100 hover:text-emerald-600 text-gray-500 rounded-2xl transition-all flex items-center justify-center"
                                    >
                            ...
                            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center gap-2">
                                ...
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Contact student for schedule</p>
                            </div>
                        </div>
                    ))}
                </div>
